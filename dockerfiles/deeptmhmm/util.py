...
def set_experiment_id(data_set_identifier, learning_rate, minibatch_size, experiment_id=None):
    if experiment_id:
        globals().__setitem__("experiment_id", experiment_id)
    else:
...
def write_model_to_disk(model, experiment_tag=''):
    if experiment_tag:
        path = "output/experiments/" + experiment_tag + '/' + globals().get("experiment_id") + ".model"
    else:
